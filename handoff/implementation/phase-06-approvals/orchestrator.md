...
- Post-Phase-6 schema cleanup for legacy `Draft.note` was completed separately in `handoff/implementation/phase-06-draft-note-cleanup/` after approvals validation confirmed no hidden dependency remained.
...
- Those fixes are tracked separately in `handoff/implementation/phase-06-approvals-followup/` so the original Phase 6 closure remains readable while the follow-up trail stays explicit.
